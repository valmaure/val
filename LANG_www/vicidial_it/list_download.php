...
elseif ($download_type == 'dnc')
	...
	if ($leads_count < 1)
		{
		echo "There are no leads in Campagna DNC list: $group_id\n";
		exit;
		}
	}
elseif ($download_type == 'fpgn')
	...
	if ($leads_count < 1)
		{
		echo "There are no leads in this filter phone gruppo: $group_id\n";
		exit;
		}
	}
else
...
if ( ($custom_fields_enabled > 0) and ($event_code_type=='LIST') )
	...
	if ($list_id=='ALL-LISTE')
		...
		while ($lists_ct > $u)
			{
			$stmt="MOSTRA TABELLAS LIKE \"custom_$custom_list_id[$u]\";";
			if ($DB>0) {echo "$stmt";}
			$rslt=mysql_query($stmt, $link);
